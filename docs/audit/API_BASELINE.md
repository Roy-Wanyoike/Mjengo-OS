...
| AI (`/api/ai/*`) | 7 | 9 | voice-log, parse-text, analyze-photo, authenticity-screen (GET+POST), recap, anomaly-scan, extract-document (POST+PUT) — audit-time count; #153 later added extract-document GET (10 method-endpoints) |
...
- **OpenAPI** (`src/app/api/openapi.json/route.ts`): **29 documented paths** (27 `/api/v1/**` + `/api/audit` + `/api/reports/budget-variance`). **All 29 exist with exactly the documented methods — zero documented-but-missing, zero v1 implemented-but-undocumented.** The remaining 31 paths are intentionally undocumented (the doc's description scopes it to "exactly the 27 /api/v1 route paths … plus the two wave-3 app-level GETs", route.ts:1499-1505). ARCHITECTURE.md:18-20 states the same count (29) — consistent.
  - *2026-09-19 update (#153):* the document-intelligence route `/api/ai/extract-document` (GET queue / POST draft / PUT review) joined the doc — **30 documented paths**; the scope note now names it and the still-undocumented `/api/ai/*` remainder (API-14 keeps tracking the wider extension).
  - *2026-09-23 update (#165 / API-14):* the scope decision is now recorded as **ADR 0008** (`docs/adr/0008-openapi-scope.md`) — v1 + the three enumerated app reads stay the doc's scope; the app mutation surface (actions/sync) waits on API-10's payload registry, and every other undocumented family keeps its contract at its seam (ADR pointer table). The §3 cross-check below is no longer a per-audit ritual: `tests/unit/openapi-cross-check.test.ts` enforces documented ⇒ implemented (per method), v1 implemented ⇒ documented (per method), and the exact app-read set — the audit's manual finding "both sides are enumerated" is now a CI invariant.
...
| `/api/health` | GET | inline (`src/app/api/health/route.ts:60`) | public (minimal liveness — `{ok,db,timestamp}`); full detail gated: admin session OR `X-Health-Detail` + `HEALTH_DETAIL_TOKEN` OR `HEALTH_PUBLIC_DETAIL` (issue #164 / API-13) | — | — | — | read ($queryRaw always; groupBy/counts only on the gated path) | — | — |
| `/api/openapi.json` | GET | inline (route.ts:2724) | public (by design) | — | — | — | no | — | — |
| `/api/actions` | POST | `actions.ts:59` | session OR share token (CLIENT_ACTIONS only); client/supplier pins; action-family flag gate | 60/min | 1 MB | manual (`type` required; payload typed by appliers) | read+write | yes (`Idempotency-Key`/`x-idempotency-key`, replay + 409-pin BE-6) | yes (withAuditContext → logAudit, 1 row/action; #218: milestone/variation/payment `.decide` rows additionally carry the decision payload in meta + the frozen status transition in before/after) |
| `/api/audit` | GET | `audit.ts:83` | session+roles[admin] | 60/min | — | manual (query params, keyset cursor) | read only (immutable by design) | — | reads AuditEvent (meta/before/after parse back as JSON; #218 decision actions populate before/after; the admin audit tab renders the state change, null-tolerant for older rows) |
...
| `/api/notifications` | GET, POST, PUT | `notifications.ts:117,180,308` | session; client pinned; supplier row-pinned to served projects (BE-3/BE-12); owner roles MUST pass `?projectId` — 400 when absent (#157) | GET 60/min, POST/PUT 30/min | PUT: reject-mode | manual (ids≤200; prefs≤20 kinds) | read+write | — | no |
...
| `/api/search` | GET | inline (`src/app/api/search/route.ts:251`) | session; supplier 403 (BE-3); client pinned | 60/min (BE-11) | — | manual (q≤100, LIKE-wildcards stripped) | read (SQL LIKE pushdown — 10 tables, ≤300 **matches**/table, #163) | — | — |
...
| `/api/upload/confirm` | POST | `upload-confirm.ts:55` | session+roles[contractor,admin,client] | 10/min | 64 KB | zod (key regex, category) | write (Attachment) — **idempotent on `objectKey` (#159)**: replay → 200 `replayed:true` w/ original row, same-key-different-category → 409 | — | — |
...
| `/api/ai/extract-document` | POST, PUT | `modules/documents/service.ts` | none | 128 KB (hint ≤100 KB) | write (Attachment extraction / review) | `simulated:false` label; PUT = human review gate, logs AuditEvent. *#153 later added the GET review-queue read (30/min, projectId-required) that feeds the Copilot "Documents" panel* |
...
- **Documented-but-missing: 0. Implemented-v1-but-undocumented: 0.** *Since #165 this pair of checks is automated — `tests/unit/openapi-cross-check.test.ts` runs the doc's paths×methods against the route files on every CI pass (documented ⇒ implemented per method, and the v1 reverse direction), plus the ADR 0008 scope pin (doc paths == disk-walked v1 + exactly the three app reads).*
- Undocumented-by-design (31 paths → 30 since #153 moved extract-document in): the whole legacy/AI/upload/push/webhook surface. The doc's `info.description` says this explicitly and links **ADR 0008** (`docs/adr/0008-openapi-scope.md`), which records the decision and the pointer table for where each excluded surface's contract lives. Self-documenting substitutes exist for several (`GET /api/ussd`, `GET /api/whatsapp`, `GET /api/webhooks/daraja` return machine-readable contracts; `src/backend/README.md:72-73` lists the module map).
...
| `inventory` | 4 / 1,167 | **WORKING** | atomic upsert + StockMovement append, closing-qty result contract (`service.ts:1-21`); #207 honest lowStock — ONE rule in pure `low-stock.ts` (explicit `reorderLevel` when set, else closing ≤ 10% of opening+received+returned, zero-inflow never low), flag computed in the slice + v1 materials rollup, crossing writes one `stock.low` notification per crossing (never per read); #147 `updateQuote` header edit + deleteMany/recreate line rewrite in ONE `$transaction` — mid-rewrite failure rolls back to the original quote (pinned stub + real-engine) |
...
| API-2 | **P2** | `/api/ai/extract-document` (POST + PUT review gate) has **no frontend consumer** and is absent from the OpenAPI doc — an orphaned, tested API surface; the document-review decision flow has no UI. | grep `extract-document` in `src/frontend` → 0 hits; openapi.json paths list; only `tests/unit/extract-document-pdf.test.ts` exercises it | "Wire or document the document-intelligence API (extract-document) consumer surface" — **RESOLVED 2026-09-19 via #153** (wired, option (a): Copilot "Documents" review panel + GET review-queue read + OpenAPI path) |
| API-3 | **P2** | v1 project-scoped list routes call `getProjectPayload(id)` — a **full ~15-table aggregation** — then filter/slice one small collection in memory. `/api/v1/projects/{id}/tasks` etc. pay the whole-payload cost per page. Acknowledged in code ("heavyweight read … rate limit 120/min") but is O(payload) per page, not O(page). | `v1/project-tasks.ts:41`, `v1/project-workers.ts:55`, `v1/project-parcels.ts:64`, `v1/project-milestones.ts:52`, `v1/project-detail.ts:52` | "v1 project-subresource reads should query the subresource directly instead of the full payload" — **RESOLVED 2026-09-19 via #154**: tasks/milestones/workers/parcels/invoices read ONE direct `findMany` scoped `where:{projectId}` with their filter, the keyset boundary and `take: limit+1` pushed in (the #155 attendance pattern; workers adds ONE windowed attendance read of the page's workers over an 8-day rollup window); suppliers read a new `loadSupplierDirectoryBounded` (directory take-capped at 200 + the two small relationship-mark reads); intel reads `loadIntelSlice` directly + the route-layer Alert read; `getProjectPayload` remains only the webapp's `/api/project` read (project-detail keeps it legitimately). Responses byte-identical, tenant pins unchanged, query shapes pinned by `tests/unit/v1-direct-reads.test.ts` (incl. the getProjectPayload-never-called spy) |
| API-4 | **P2** | Several core reads are **unbounded Prisma queries** paginated only in memory: `db.project.findMany` (no take) in `getProjectsList`; milestones/variations/siteZones/photoComments in `getProjectPayload`; `db.attendance.findMany` (no take) in v1 attendance route; `loadSupplySlice` loads all orders+deliveries. Safe at current scale (SQLite, bounded demo data, documented "bounded in practice") but no DB-level limit exists. | `lib/mjengo.ts:164-172,236-243`; `v1/project-attendance.ts:64`; `v1/supply-orders.ts:72` | "Add take limits / keyset pushdown to portfolio and subresource reads" — **RESOLVED 2026-09-18 via #155**: roster scan take-capped at 500 + `/api/projects` GET DB-level keyset (cursor on createdAt/id, additive `nextCursor`/`hasMore`, id-scope pushdown for the client/membership pins); payload reads take-capped (milestones 200 / variations 60 / zones 120 / photoComments 120 — aggregate-feeding scans left uncapped BY DESIGN with the #144 SQL-aggregate follow-up recorded in-code); v1 attendance pushes filters + boundary + `take: limit+1` into the `findMany` (response byte-identical); v1 supply orders/deliveries lists read a new `loadSupplyOrdersBounded` (take 200) while detail surfaces keep the full slice; deliberate unbounded reads state their bound-in-practice argument next to the query |
...
| API-6 | P3 | `/api/notifications` GET for **non-client, non-supplier roles defaults to the FIRST project in the DB** when no `?projectId` — a cross-project default read (owner roles are trusted portfolio-wide, so not a tenant breach, but surprising). | `notifications.ts:227-233` ("portfolio default (first project)") | "Drop the first-project default on /api/notifications GET" — **RESOLVED 2026-09-18 via #157**: owner roles now MUST name `?projectId` — a missing param is an honest 400 `projectId required — owner roles have no default project` (zero notification rows read, zero first-project lookup), replacing the arbitrary oldest-`createdAt` adoption; client pins and the supplier served-union/explicit branches are byte-unchanged; the webapp's only GET caller (the settings-tab prefs card) always sends the active project's id, so the 400 is unreachable in-app. Pinned by `tests/unit/supplier-role.test.ts` (the flipped MIRROR + the #157 owner-contract describe: all six owner roles + an unknown role → 400, explicit project → its rows, unknown project → 404) |
...
| API-8 | P3 | `/api/upload/confirm` is **non-idempotent by design** — confirming the same key twice creates two Attachment rows pointing at one object; the dedupe seam "would need a schema index (out of scope)". | `upload-confirm.ts:24-28` | "Add a unique index on Attachment upload key (or dedupe) for confirm flow" — **RESOLVED 2026-09-19 via #159**: natural-key route — new nullable `Attachment.objectKey` (the RAW `upp-…` key, NOT `storageKey`: that column records `publicUrl(key)`, a per-call presigned GET on private-bucket deployments, so it is not stable enough to dedupe on) with a unique index (migration 18, additive by construction — every pre-existing row is NULL and SQLite uniques skip NULLs, so no dedupe pass); the route replays on the pre-create lookup AND on the P2002 race path (concurrent double-confirm → both 200, one row — what the #177 IdempotencyRecord seam cannot deliver, being check-then-act on a side table), returns the original row with `replayed: true`, and refuses a same-key/different-category retry 409 without replaying or reclassifying. Pinned by `storage-presign-routes.test.ts` (route branches) + `upload-confirm-realdb.test.ts` (real engine, real P2002 shape) + `db-integrity-constraints.test.ts` (migration-level, incl. the duplicate-seeded pre-#159 DB) |
...
| API-12 | P3 | `/api/search` is an in-memory LIKE scan of ≤300 recent rows per table — rows older than the window are "honestly missed"; fine now, but search quality/scale ceiling is structural. | `src/app/api/search/route.ts:49-56` | "Move global search to indexed SQL LIKE / FTS when data grows" — **RESOLVED 2026-09-24 via #163 (the pushdown, not the index):** the LIKE now runs INSIDE every one of the ten source queries (Prisma `contains` → SQLite `LIKE '%q%'`, ASCII case-insensitive by default — no `case_sensitive_like` pragma is set; pinned on the real engine) with the SAME `take: 300` bounds and recent-first orders, so the window caps **matches** per table (the ≤300 most-recent matches) instead of raw rows: an exact-name hit is found at any age — the silent-miss failure mode ("a project exists, the user types its exact name, and gets nothing") is gone, and the #166 projects-window inversion that made it worst on the newest projects was already fixed there. A miss now needs >300 matches for one query, and when that cap truncates, the response carries an honest `note` (the same seam the query-cap/min-char branches already use; additive — the ⌘K palette reads `ok`/`groups` only). Wildcard-stripping is unchanged and now load-bearing end-to-end: Prisma does NOT escape `%`/`_` inside `contains` on SQLite (a raw `contains 'a%z'` wildcards — demonstrated and pinned), so `sanitize()` is what keeps user wildcards literal. One documented narrowing: LIKE folds case for ASCII only, where the old in-memory `toLowerCase()` folded Unicode (app data is English/Swahili — ASCII; the route header has claimed "ASCII case-insensitive" since the route was born, and the pushdown makes it literally true). FTS5 remains the recorded scale-out path — revisit trigger: any searchable table >10k rows (ARCHITECTURE.md roadmap + route header decision record; a schema change, to be coordinated with the DB waves). Pinned by `tests/unit/search-pushdown.test.ts` (route-level: the ten pushed-down where shapes with take/order/includes preserved, sanitize-before-LIKE, unchanged response shape, the note seam at exactly-300-matches, client pinning through the where clauses), `tests/unit/search-pushdown-realdb.test.ts` (real engine: parity vs a frozen replica of the OLD algorithm over a shared fixture battery — same rows, same order; the beyond-window oldest-row find the old algorithm missed; the 300-match cap + note; the ASCII case-insensitivity linchpin; the raw-wildcard demonstration) and `tests/unit/search-window-order.test.ts` (#166 desc ordering, updated to the pushed-down call shape and the new match-window ceiling) |
| API-13 | P3 | `GET /api/health` publicly exposes coarse DB counts (projects/workers/notifications) and package version. Documented ("coarse counts"), low risk, worth noting for customer-facing deployments. | `src/app/api/health/route.ts:7-20` | "Consider gating detailed /api/health fields behind an internal header" — **RESOLVED 2026-09-18 via #164**: the public body is now the probe minimum `{ok,db,timestamp}` (503 stays public/minimal — probes and the compose/CI healthchecks see the same status + ok/db they always did; the public 503 no longer carries the DB error text); the full pre-split detail (counts, jobs, version, uptime, dbLatencyMs — and the 503 error text) is served only through one of three gates: an admin session (the in-app SystemHealthCard keeps working unchanged), `X-Health-Detail: <HEALTH_DETAIL_TOKEN>` (constant-time compare via the jobs/run helper, fail closed when unset), or the `HEALTH_PUBLIC_DETAIL=1` demo opt-in; the count queries run only on the gated path (probe hot path = one `SELECT 1`). Both shapes pinned by `tests/unit/health-route.test.ts` |
| API-14 | P3 | OpenAPI covers only v1 + 2 app GETs; the AI, upload, push, sync and actions surfaces — the ones the webapp itself uses — have no machine-readable contract (only prose GET-contracts on the gateway routes). | openapi.json:1499-1505 | "Extend OpenAPI (or a second doc) to the legacy app surface" — **RESOLVED 2026-09-23 via #165 (option b, the honest close):** the scope is now a decision, not an omission — **ADR 0008** (`docs/adr/0008-openapi-scope.md`) records it with a pointer table naming where every excluded surface's contract lives (actions → the `ActionType` union's 124 per-action payload-shape comments + `src/shared/*` allowlists; sync → the conflict-ladder header; upload/push → route zod `strictObject`s; AI remainder → `enforceAiRoutePolicy`; webapp-private reads → this audit's §2.1 inventory; ussd/whatsapp/daraja → their runtime GET contracts), and with revisit triggers (API-10 registry landing, a mobile/second consumer, an integrator program). Extension of the actions/sync family was deliberately NOT taken now: their payloads are `any` at the route (API-10), so an OpenAPI entry would be an unvalidatable contract and hand-maintaining ~124 payload shapes would drift. `info.description` links the ADR; ARCHITECTURE.md's API surface references it; and the §3 cross-check is automated as `tests/unit/openapi-cross-check.test.ts` (documented ⇒ implemented per method, v1 implemented ⇒ documented, exact app-read pin — rigor-checked to fail on an undocumented v1 route, a documented-but-missing path, and method-level drift) |
...
- **Orphaned endpoints (implemented, no `src/frontend` consumer)**:
  ...
  - `/api/ai/extract-document` (POST+PUT) — see API-2. *Resolved by #153 (2026-09-19): the Copilot "Documents" panel now consumes all three verbs via `frontend/mjengo/copilot/document-review.ts`.*
...
- BE-3/BE-6 supplier scoping + client replay pin: `notifications.ts:141-173,206-239`; `search` route 403; `jobs.ts:71`; actions replay pin `actions.ts:126-128`. ✔
