...
- Error redaction: `safeErrorMessage`/`isInternalError` strip Prisma/framework internals from client bodies (`guard.ts:64-83`); v1 uses `mapServiceError` with the same doctrine; s3-compat errors are secret-free by construction.
...
- **next-auth ^4.24.15 on Next ^16.1.1 + React 19** (package.json:84-92): v4 predates Next 15/16; the App-Router handler is shimmed with a cast (`[...nextauth]/route.ts:25-33`). v4 is maintenance-mode upstream; the v5 line is the supported one for this Next generation. Auth core on an unsupported framework pairing = SEC-5 (P2).
...
| SEC-5 | P2 | next-auth v4.24.15 + Next 16/React 19: unsupported pairing for the authentication core (cast-shimmed route handler) | package.json:84-92; [...nextauth]/route.ts:25-33 |
