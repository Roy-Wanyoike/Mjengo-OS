...
- Lockout: 5 wrong PINs / 15 min → 15-min lock, keyed **per caller-supplied `phoneNumber`** (shared tracker store, `rate-limit.ts:431-490`); correct PIN clears (consecutive semantics). Per-IP 40/min PIN throttle covers phone-number rotation (`ussd/route.ts:276-291`) — but the phone-keyed lockout budget is attacker-refreshable (SEC-9, P3). **RESOLVED (#176, 2026-09-18): the lockout is rekeyed on non-attacker-supplied values** — PRIMARY the trust-aware client-IP principal (`ip:` tracker keys; TRUST_PROXY unset → the ONE shared `anon` budget, so rotating MSISDNs or XFF values refreshes nothing, and a correct PIN no longer clears the budget — the phone-tail refresh vehicle is closed); SECONDARY the resolved worker identity (`worker:` keys) for kiosk-PIN attempts (wrong PINs attributed via the caller's line accumulate per worker regardless of phone/IP; a locked worker is refused from any phone; only a correct KIOSK pin clears the worker's own count). Wrong-PIN rate is now bounded by the lockout (5/15 min per principal), not the 40/min bucket. Honest residual: a missed guess names no target worker, so per-worker accumulation keys on the line (aggregator-vouched under the shared secret; the ip budget bounds the open posture), and a real aggregator's multiplexed gateway IP shares one budget (availability/brute-force trade-off documented in the route contract).
...
| SEC-9 | P3 | USSD PIN lockout keyed on caller-supplied phone number — lockout budget refreshable by rotating MSISDNs; per-IP 40/min is the effective bound. **RESOLVED (#176, 2026-09-18): rekeyed on the client-IP principal (primary) + resolved worker identity (secondary); rotating phones/worker targets can no longer refresh the budget and a correct PIN no longer clears it** | rate-limit.ts:444-490; ussd/route.ts:276-291 (pre-#176 evidence); createUssdPinLockout + the route's lockout gates (post) |
...
- Webhooks: Daraja secret-path + IP allowlist + query-API reconciliation + durable dedupe rated strong; USSD/WhatsApp HMAC optional with warn-only production posture (SEC-4); USSD lockout keyed on caller-supplied phone (SEC-9) — since rekeyed on client-IP + worker identity (#176).
