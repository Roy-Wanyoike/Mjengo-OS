...
1. **CI has never executed** (billing lock, issue #98) — restore billing or mirror gates to a free runner; until then every merge's green light is a local claim (INF-4). The container-smoke half of INF-4 landed 2026-09-18 via #198 (docker.yml `smoke` job: compose-up + /api/health probe); its first real run also waits on #98.
...
5. **No error tracking or alerting** — no Sentry-class sink; failed jobs/notifications only visible if someone looks — **the opt-in error sink seam landed 2026-09-19 via #202** (`ERROR_SINK_URL` webhook v1, fail-open, redacted payloads; unconfigured = journal-only, the historical default); uptime monitoring/alerting (item 4) remains open.
6. **No log aggregation / structured logs** — console text only; no correlation IDs on log lines (OBS-2) — **structured logs + requestId correlation landed via #204/PR #277** (`lib/log.ts`; documented in DEPLOYMENT §10.1, added with #202); SHIPPING the logs off-box (aggregation) remains the operator's move.
...
| INF-4 | Medium | CI has never run (billing lock #98): all gates locally-verified; docker.yml builds but never runs/smoke-tests an image; no registry/artifact publishing — **smoke test landed 2026-09-18 via #198** (docker.yml `smoke` job: compose-up of both images with dummy secrets on a throwaway SQLite volume, bounded GET /api/health poll to `{"ok":true,"db":"up"}`, prisma success-line assertion from the boot log, /website rewrite probe, guaranteed teardown; first execution rides the first green run after #98 — registry/artifact publishing remains open for INF-12) | "Restore CI execution (billing) + add container smoke test to docker.yml" — smoke-test half done (#198), billing still blocked (#98) |
...
| OBS-1 | Medium | No error tracking (Sentry-class); prod failures visible only in console/journal/JobRecord rows — **landed 2026-09-19 via #202** (opt-in fail-open error sink seam: `lib/errors/sink.ts` webhook v1 on the #204 log substrate, wired into route-kit/jobs/webhook catches; unconfigured = journal-only default unchanged) | "Add error tracking sink (opt-in env) for API + job failures" — done (#202) |
| OBS-2 | Medium | Unstructured console logs; no correlation IDs on log lines (requestId only in audit rows); no request access log — **landed 2026-09-19 via #204/PR #277** (`lib/log.ts` seam: JSON in prod / text elsewhere, requestId propagation via withRequestLogging, drain-run ids, access lines) | "Structured JSON logging + request-ID log correlation" — done (#204) |
