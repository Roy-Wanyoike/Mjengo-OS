...
| WD-9 | INFO | `NEXT_PUBLIC_SITE_URL` omitted at build → all absolute URLs fall back to `http://localhost:3001` (documented in Dockerfile/.env.example as MW-9; watch item for whoever deploys an indexed site). — **FIXED 2026-09-19 via #149: promoted from doc footnote to launch gate — DEPLOYMENT.md §6.7 "Website launch gate" checklist (set `NEXT_PUBLIC_SITE_URL` at build before serving an indexed site + verify the bake via sitemap curl) + a build-time `[site-url]` warning in `next.config.ts` (decision pure in `lib/site.ts` `siteUrlBuildWarning`, pinned by `tests/unit/website-siteurl-gate.test.ts`) that fires for the one "indexed site, localhost URLs" combination — production standalone build with no usable `NEXT_PUBLIC_SITE_URL`; integrated zero-override default and dev stay silent; warn-only, never a build failure** | `lib/site.ts:36`; `Dockerfile:60-67` | "Website: SITE_URL localhost fallback — promote from doc footnote to launch-gate watch item" — done via #149 |
