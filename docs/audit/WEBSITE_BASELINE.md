...
`app/pricing/page.tsx` + `data/pricing.ts`: Early Access tier highlighted "Current stage" (free during pilot, ≤3 projects soft limit, Nairobi & Kiambu first); **Team (KES 15,000/project/month) and Portfolio (custom) are explicitly footnoted "Indicative — final pricing will be published before the pilot ends"**. FAQ states wallet holds no money, M-Pesa initiation "in sandbox testing; production transfers are not enabled yet". This is consistent with the single-operator/Kenya-sandbox rails posture in the OS app (wallet = ledger + ESCROW ledger account, Daraja sandbox — `src/backend/modules/wallet/daraja.ts`, `src/backend/api/v1/project-escrow.ts`). One wording tension: "Escrow-style releases" (WD-3) — **resolved 2026-09-19 via #138 (approval-gated language everywhere; see §9 WD-3)**. Pricing tables are list-based (a11y-safe), `lg:grid-cols-3` responsive.

## 5. SEO status

- **`app/layout.tsx:20-65`**: `metadataBase` = `SITE.url + SITE.basePath` (MW-9 aware); title template `%s — MjengoOS`; 9 keywords; OG 1200×630 `/images/og.png` (real file, `public/images/og.png`); Twitter `summary_large_image`; robots index/follow; icons favicon.ico + icon-192.png; `viewport.themeColor #123C32`. Every page exports `metadata` with unique title/description + `alternates.canonical`. 404 is noindex. — **2026-09-25 update (#142): icons are now favicon.ico + 192/512 PNGs, all basePath-aware via `asset()`** (Next passes icon/manifest hrefs through verbatim — only canonicals/OG get the metadataBase join), plus `metadata.manifest` → `public/manifest.webmanifest`.
- **`app/sitemap.ts`**: 24 URLs (19 routes incl. 6 slugs) with priorities; `lastModified` **derived at build time via #143** (3-rung ladder: `SITEMAP_LAST_MODIFIED` build ARG → `git log -1` site-touching commit → omitted entirely; WD-6 fixed). **`app/robots.ts`**: allow all, disallow `/api/`, sitemap link — both join origin+basePath correctly.
...
- Icons: `public/icons/icon-512.png` shipped but **unreferenced**; SEO.md:10 claims "192/512 PNGs" wired; no `manifest.webmanifest` (root app has one, website doesn't) → WD-5. — **RESOLVED 2026-09-25 via #142**: `public/manifest.webmanifest` added (name/short_name/description matching `lib/site.ts`, theme `#123C32` = viewport themeColor, paper `#f3f2ee` background, standalone display, RELATIVE `start_url`/`scope`/icon srcs so one file serves `/` and `/website` alike), `metadata.manifest` + favicon/192/512 icon hrefs wired through `asset()` (integrated mode previously emitted bare `/favicon.ico` + `/icons/icon-192.png`, which 404'd on the site server and collided with the OS app's differently-branded root icons on the shared origin), SEO.md icon/manifest section now matches reality; pinned by `tests/unit/website-pwa-manifest.test.ts`.
...
- **Marketed-vs-real cross-check**: `/about` "Live in the product today" list (lines 88-99) maps to real OS-app surfaces (v1 API family, evidence/upload routes, attendance+USSD PIN, invoices/M-Pesa references, land module, notify module, AI routes flag-gated, SW offline sync, supplier portal/supply orders, `/api/v1/*` + openapi.json). Only wording flag: WD-3 "Escrow-style" — resolved 2026-09-19 via #138 (approval-gated language).
...
| WD-3 | LOW | "Escrow-style milestone releases" (3 marketing spots) vs `/wallet` "There is no escrow custody" — hedged but tension-prone; app implements an ESCROW ledger account + approval gates, not custody. Suggest "approval-gated milestone releases". — **FIXED 2026-09-19 via #138: all three spots now "Approval-gated" (`data/roles.ts` client gain, `module-grid.tsx` description + cap chip, `approvals-deep-dive.tsx` bullet); `escrow` survives on the website only as negations (/wallet honesty band, /terms) and the deliberate DemoChip demo datum "Escrow top-up" (`sections/wallet.tsx` — the ESCROW ledger-account name the app genuinely has); app wallet copy aligned in the same stroke (`money.wallet.note` + `money.topup.desc`, en+sw — approval-gated/committed phrasing; account-name labels like "MjengoPay escrow wallet" unchanged); pinned by `tests/unit/website-escrow-copy.test.ts`** | `data/roles.ts:32`; `app/platform/components/module-grid.tsx:64-65`; `app/platform/components/approvals-deep-dive.tsx:20`; vs `app/wallet/components/honesty-band.tsx:26` | "Website copy: replace 'Escrow-style releases' with 'approval-gated releases' to match the not-a-bank disclosure" — done via #138 |
...
| WD-5 | LOW | `public/icons/icon-512.png` unreferenced (layout references only favicon + 192); SEO.md claims "192/512 PNGs"; no `manifest.webmanifest` for the website. — **FIXED 2026-09-25 via #142: manifest added (relative start_url/scope/icon srcs — mode-agnostic), icon-512 + manifest wired through `asset()` in `layout.tsx`, SEO.md truth restored; see §5 update** | `app/layout.tsx:58-64`; `SEO.md:10`; `ls public/icons/` | "Website PWA basics: wire icon-512 + add manifest.webmanifest; fix SEO.md icon claim" — done via #142 |
| WD-6 | LOW | Sitemap `lastModified` is a frozen constant (2026-09-09) that must be hand-bumped per deploy; `changeFrequency: "monthly"` for all pages regardless of reality. — **FIXED 2026-09-19 via #143: `lastModified` now derived at build time (`SITEMAP_LAST_MODIFIED` build-ARG override → `git log -1 --format=%cI -- .` last-commit-touching-site → omitted entirely when neither exists — never hand-bumped); `changeFrequency` removed (Google ignores it; uniform "monthly" was noise)** | `app/sitemap.ts:10,49-51` | "Sitemap: automate/verify lastModified bump (stale since 2026-09-09)" — done via #143 |
| WD-11 | LOW | Backup guidance (DEPLOYMENT §7.2) covers SQLite + uploads volume but **not** the `website-data` leads volume (plaintext PII). — **FIXED 2026-09-26 via #151 (in two halves): #199's scheduled backup already tars the volume (`mjengo-backup.sh` `MJENGO_WEBSITE_DIR` + `MJENGO_BACKUP_WEBSITE=1` on by default, `umask 077`; §7.2/§7.2.1/§7.2.2 + a §6.3 cross-ref all name it), and #151 closed the guidance gap — the §7.2 manual ad-hoc path now ships a working one-liner (`docker compose exec -T website tar -C /app/data -cf - . > website-data-$(date +%F).tar`, with the `-T`-or-TTY-corruption rationale + a host-side variant), §7.2.1's cadence note ties backup frequency to the 500-entry cap ("at least as often as your §6.3 retrieval cadence"; a >500 burst inside 24 h evicts leads no backup ever saw), the PII note gained Kenya DPA 2019 handling (retention-as-expiry ~28 d, narrow access, live-file-first erasure), and §7.2.2's verify step reads `submissions.json` back through the site's own path; pinned by `tests/unit/website-pii-backup.test.ts` incl. a real tar round-trip** | `DEPLOYMENT.md:450-463` vs `:280,309` | "Add website-data (submissions.json) to the documented backup set — it is leads/PII" — done via #151 |
