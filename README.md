...
- **Tests** (`test.yml`) — the full vitest unit suite, **`bun run test`**
  (3,126 tests across 141 files, all passing — counts as of 2026-09-26;  re-run `bunx vitest run` for the current number, since every wave adds  tests), on every push/PR to `main`. No database or secrets required —
