...
- **Tests** (`test.yml`) — the full vitest unit suite, **`bun run test`**
  (3,111 tests across 139 files, all passing — counts as of 2026-09-19;  re-run `bunx vitest run` for the current number, since every wave adds
...
| `tests/unit/` | The vitest suite (139 files, 3,111 tests — counts as of 2026-09-19; re-run vitest for current) — unit-level, no DB or secrets needed (critical-path suites use a throwaway real SQLite file) || [ARCHITECTURE.md](./ARCHITECTURE.md) | Module map + production migration roadmap |
