...
// Delivery receive dialog (Finder §13 — the physical ground-truth moment).
// Per-line ordered vs received counts, photo count, GPS capture via
...
// server writes the per-line OrderDeliveryLine rows and notifies client +
// contractor. TODO(photos): v1 records a count — real photo attach lands with
// object storage (roadmap A-5).
//
...
import type { OrderWithDetail } from '@/modules/supply/types'
...
function DeliveryReceiveForm({
  ...
  async function save() {
    const lines = order.lines.map((l) => ({ orderLineId: l.id, qtyReceived: Number(counts[l.id]) }))
  ...
  return (
    <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-md">
      <DialogHeader>
        ...
        <DialogDescription>
          The physical count is ground truth: count what actually arrived, line by line. Short counts are flagged
          for review — never accusations; payment stays gated by the 3-way match.
        </DialogDescription>
      </DialogHeader>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-stone-400">
            Ordered vs received — {order.supplierName}
          </p>
          {order.lines.map((line) => {
            const received = Number(counts[line.id])
            const short = Number.isFinite(received) && received < line.qty
            return (
              <div key={line.id} className="grid grid-cols-[1fr_6.5rem] items-center gap-2 rounded-lg border border-stone-200 px-3 py-2">
