...
/**
 * Offline-first sync endpoint (src/app/api/sync/route.ts is the shim).
 * The device queues actions locally (simulated on-device SQLite outbox) while
 * offline, then flushes them here when connectivity returns. Each action is
 ...
 * limit on /api/actions would be bypassed) + the body parse + the generic
 * 500 'Sync failed' catch (body errors historically landed in that catch).
 */
