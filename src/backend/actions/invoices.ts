...
export const INVOICE_ACTIONS = [
  ...
  'invoice.pay', // { id, method: 'mpesa'|'bank'|'card'|'wallet'|'cash', reference?, costCode?, phaseId?, acknowledgeMismatch?, by? } — provider seam + double-entry ledger (ledgerTxnId on the Transaction row; optional phaseId stamps the real phase cost-code, issue #39 — validated in-project, foreign phase refused)
