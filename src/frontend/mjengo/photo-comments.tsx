...
function RoleChip({ role }: { role: string }) {
  const t = useT()
  if (role === 'client') {
    return <span className="inline-flex items-center rounded-full border border-amber-200 bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-800">{t('comments.role.client')}</span>
  }
  if (role === 'foreman') {
    return <span className="inline-flex items-center rounded-full bg-stone-600 px-1.5 py-0.5 text-[10px] font-medium text-stone-50">{t('comments.role.foreman')}</span>
  }
  return <span className="inline-flex items-center rounded-full bg-stone-800 px-1.5 py-0.5 text-[10px] font-medium text-stone-50">{t('comments.role.contractor')}</span>
}
 ...
 * All copy flows through useT() (comments.* — issue #125).
 */
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="h-14 w-20 shrink-0 overflow-hidden rounded-lg border border-stone-200 bg-stone-100">
          <img src={photo.url} alt={photo.caption ?? t('comments.sitePhoto')} className="h-full w-full object-cover" />
        </div>
        <div className="min-w-0">
          <p className="truncate text-sm font-medium text-stone-900">{photo.caption ?? t('comments.sitePhoto')}</p>
          <p className="text-xs text-stone-500">
            {comments.length === 1
              ? t('comments.countOne', { count: comments.length })
              : t('comments.countMany', { count: comments.length })}
            {unresolved > 0 ? ` · ${t('comments.open', { count: unresolved })}` : ''}
          </p>
        </div>
      </div>
      ...
      {sorted.length === 0 ? (
        ...
        <ul className={`max-h-72 space-y-2 overflow-y-auto pr-2 ${SCROLLBAR}`} aria-label={t('comments.aria')}>
          {sorted.map((c) => (
            <li
              key={c.id}
            ...
            >
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                ...
                {c.resolved && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-medium text-stone-500">
                    <CheckCheck className="w-3.5 h-3.5 text-amber-600" aria-hidden /> {t('comments.resolved')}
                  </span>
                )}
              </div>
              ...
              {!c.resolved && canResolve && onResolve && (
                <Button
                  ...
                  aria-label={t('comments.resolveAria', { author: c.author })}
                  onClick={async () => {
                    setResolvingId(c.id)
                    try {
      ...
      {onAdd && (
        <div className="space-y-2">
          <Textarea
            ...
            placeholder={t('comments.ph', { author: defaultAuthor, role: t(`comments.role.${defaultRole}`) })}
            className="min-h-[72px] bg-white"
            aria-label={t('comments.addAria', { author: defaultAuthor })}
            onKeyDown={(e) => {
              if (e.key === 'Enter' && (e.metaKey || e.ctrlKey)) void submit()
            }}
          />
          <div className="flex items-center justify-between gap-2">
            <p className="text-[11px] text-stone-400">{t('comments.sendHint')}</p>
            <Button
              size="sm"
              ...
              onClick={() => void submit()}
            >
              {sending ? <Loader2 className="w-4 h-4 animate-spin" aria-hidden /> : <Send className="w-4 h-4" aria-hidden />} {t('comments.send')}
            </Button>
          </div>
        </div>
      )}
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-stone-900">
            <MessageSquare className="w-5 h-5 text-amber-600" aria-hidden /> {t('comments.dialogTitle')}
          </DialogTitle>
          <DialogDescription>{t('comments.dialogDesc')}</DialogDescription>
        </DialogHeader>
