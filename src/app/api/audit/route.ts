 ...
 *
 * IMMUTABILITY — READ-ONLY BY DESIGN. This file deliberately exposes ONLY a
 ...
 *   · Guard: admin role only (withGuard { roles: ['admin'] }) → 401/403.
 *   · Rate limit: 60/min per principal (enforceRateLimit 'audit.list').
 *   · Filters:
...
export const GET = withGuard(
  async (req: NextRequest) => {
    // Rate limit AFTER the admin guard: 401/403 first, then 429 — anonymous
    // brute-force never burns tokens and admins get clear auth errors.
    const limited = await enforceRateLimit(req, 'audit.list', 60, 60_000)
