...
export async function GET() {
  return NextResponse.json({
    ...
    security: {
      ...
      network:
        'Origin checks mirror route-kit\u2019s MUTATION_ORIGIN_ALLOWLIST semantics (browser Origins are checked when the env is set; Safaricom\u2019s server-to-server POSTs carry no Origin and always pass). Optional source-IP allowlisting: set DARAJA_ALLOWED_IPS (comma-separated IPv4 CIDRs, e.g. 196.201.214.0/24; IPv6 entries are exact-match only) and the callback route rejects non-matching sources with 403 BEFORE parsing the body. Unset = the documented default: the unguessable path + query-API reconciliation are the integrity model. TLS terminates at the reverse proxy in front of the app. (Safaricom publishes no stable IP list — obtain the current ranges from their developer portal before enabling this.)',
      response:
